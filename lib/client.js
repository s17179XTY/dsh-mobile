	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function DialogCard() {
				...
				// The bridge's /desktop/snapshot uses `running: true` (not a
				// `status` field), while the host's /phone-connect/bridge route
				// uses `status`. Accept both so the dialog state is correct
				// regardless of which snapshot produced `snap`.
				const running = Boolean(snap && (snap.status === "running" || snap.running === true));
				const pairingUrl = running && snap.pairingUrl ? snap.pairingUrl : null;
				React.useEffect(() => {
					if (!pairingUrl) {
						setQr(null);
						return undefined;
			...
			// settings.trigger is a SINGLE slot already registered by the
			// shipped settings shell at priority 0. Static plugins get no
			// automatic shadowing priority (unlike dynamic plugin runs), so
			// registering at the same priority throws. An explicit LOWER
			// priority shadows the shipped trigger (lowest renders) and, when
			// this plugin is removed, the shipped trigger returns untouched.
